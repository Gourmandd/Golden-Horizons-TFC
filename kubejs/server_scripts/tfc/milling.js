...
ServerEvents.recipes(event =>{
    ...
    global.TFC_GEMS.forEach(gem => {
        event.recipes.create.crushing(Item.of(`tfc:powder/${gem}`, 4), `tfc:ore/${gem}`)
            .processingTime(50)
            .id(`modpack:crushing/gem/${gem}`)
        event.recipes.create.crushing(Item.of(`tfc:powder/${gem}`, 4), `tfc:gem/${gem}`)
            .processingTime(50)
            .id(`modpack:crushing/cut_gem/${gem}`)
    })

    global.TFC_MAKES_DYE_TAGS.forEach(colour =>{
        event.recipes.createMilling(Item.of(`minecraft:${colour}_dye`, 2), `#tfc:makes_${colour}_dye`)
            .processingTime(50)
            .id(`modpack:milling/dye/${colour}`)
    })

    event.recipes.createMilling(Item.of("minecraft:light_gray_dye", 2), "tfc:plant/yucca")
        .processingTime(50)
        .id(`modpack:milling/dye/yucca`)
    event.recipes.createMilling(Item.of("minecraft:lime_dye", 2), "tfc:plant/moss")
        .processingTime(50)
        .id(`modpack:milling/dye/moss`)

    global.TFC_FLOUR_TYPES.forEach(type =>{
        event.recipes.createMilling(Item.of(`tfc:food/${type}_flour`, 1), `tfc:food/${type}_grain`)
            .processingTime(75)
    ...
    global.TFC_SAND_COLOURS.forEach(colour =>{
        event.recipes.createMilling(Item.of(`tfc:sand/${colour}`, 1), Item.of(`tfc:raw_sandstone/${colour}`, 1)).processingTime(50).id(`modpack:milling/raw_sandstone/${colour}`)
    })
        ...
        global.ORE_GRADES.forEach(grade =>{
          ...
          if (metal == "cast_iron" || metal == "copper"){
            global.RAW_METALS_ORES[metal].forEach(entry =>{
                event.recipes.createMilling(Item.of(METAL_TO_CREATE_CRUSHED_ORE[metal]).withChance((GRADE_TO_PERCENTAGE[grade] * ORE_BONUS) / 100), Item.of(`tfc:ore/${grade}_${entry}`)).processingTime(100).id(`modpack:milling/ore/${grade}_${entry}`) 
            })
          } else {
            event.recipes.createMilling(Item.of(METAL_TO_CREATE_CRUSHED_ORE[metal]).withChance((GRADE_TO_PERCENTAGE[grade] * ORE_BONUS) / 100), Item.of(`tfc:ore/${grade}_${global.RAW_METALS_ORES[metal]}`)).processingTime(100).id(`modpack:milling/ore/${grade}_${global.RAW_METALS_ORES[metal]}`)
          }
        })
    })*/
